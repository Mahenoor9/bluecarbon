...
# This function is cached to avoid repeated connection creation.
@st.cache_resource
def init_connection():
    """
...
def db_add_project(name: str, type_: str, region: str, area: float, carbon: float, status: str = "Draft") -> int:
    """
    Add a new project record to the Supabase 'projects' table.
    Calculates credits, sets timestamps, and inserts the record.
    Returns the inserted project's ID or -1 on failure.
    """
...
def db_update_status(proj_id: int, status: str) -> None:
    """
    Update the status field and updated_at timestamp of a project.
    """
    try:
...
def db_update_project(proj_id: int, updates: Dict) -> None:
    """
    Update arbitrary fields of a project, adding updated_at timestamp.
    """
    try:
...
def make_csv_template() -> bytes:
    """
    Create a CSV template for project bulk upload download.
    """
...
def validate_csv_df(df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    """
    Validates and cleans uploaded CSV data.
    Checks required columns and data types.
    Returns cleaned DataFrame and list of errors if any.
    """
    errors = []
...
def status_badge(status: str) -> str:
    """
    Create a colored badge for project status.
    """
...
def admin_dashboard():
    """
    The admin dashboard UI and behavior.
    """
...
def public_dashboard():
    """
    Public dashboard to display projects with filters.
    """
...
def main():
    """
    Main entry point that controls page navigation.
    """
